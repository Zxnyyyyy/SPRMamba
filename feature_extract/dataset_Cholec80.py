...
class ImageFolderDataset(Dataset):
    def __init__(self, vid_list_file, image_root_dir, label_dir, actions_dict, transform=None):
        ...
        # Get sorted list of image folders and label files
        # self.image_folders = sorted([f for f in os.listdir(image_root_dir) if os.path.isdir(os.path.join(image_root_dir, f))])
        # self.label_files = sorted([f for f in os.listdir(label_dir) if f.endswith('.txt')])
        #
        # assert len(self.image_folders) == len(self.label_files), "Number of image folders and labels must match!"
...
if __name__ == "__main__":
    ...
    vid_list_file = './datasets/Cholec80/splits/test.csv'
    image_root_dir = "./datasets/Cholec80/picture/"
