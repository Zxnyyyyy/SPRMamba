...
log_path = './log/ResNet50_' + str(seed) + '/'
pth_path = './pth/ResNet50_' + str(seed) + '/'
    ...
    for epoch in range(start_epoch+1, epochs):
        ...
        with torch.no_grad():
            for data in test_loader:
                ...
                test_progress += 1
                if test_progress >= len(test_loader):
                    percent = 100.0
                    print('test progress: %s [%d/%d]' % (str(percent) + '%', len(test_loader), len(test_loader)), end='\n')
                else:
                    percent = round(test_progress / len(test_loader) * 100, 2)
                    print('test progress: %s [%d/%d]' % (str(percent) + '%', test_progress, len(test_loader)),
                          end='\r')

        test_elapsed_time = time.time() - test_start_time
        test_accuracy_phase = float(test_corrects_phase) / float(len(test_loader) * val_batch_size)
        ...
        test_precision_each_phase = [f"{num:.4f}" for num in test_precision_each_phase]
        test_precision_each_phase = list(map(float, test_precision_each_phase))
        test_recall_each_phase = metrics.recall_score(test_all_labels_phase, test_all_preds_phase, average=None)
        test_recall_each_phase = [f"{num:.4f}" for num in test_recall_each_phase]
        test_recall_each_phase = list(map(float, test_recall_each_phase))

        writer.add_scalar('test acc epoch phase',
                          float(test_accuracy_phase), epoch)
        ...
        logging.info(f'test in          : {test_elapsed_time // 60:2.0f}m{test_elapsed_time % 60:2.0f}s')
        logging.info(f'test accu(phase) : {test_accuracy_phase:.4f}')
        ...
        logging.info(f"test_precision_each_phase:{test_precision_each_phase}")
        logging.info(f"test_recall_each_phase   :{test_recall_each_phase}")
        logging.info(f"test_precision_phase     :{test_precision_phase:.4f}")
        logging.info(f"test_recall_phase        :{test_recall_phase:.4f}")
        logging.info(f"test_jaccard_phase       :{test_jaccard_phase:.4f}")
        ...
        if val_accuracy_phase > best_val_accuracy_phase:
            ...
            best_te_acc = test_accuracy_phase
            best_te_pr = test_precision_phase
            best_te_re = test_recall_phase
            best_te_ji = test_jaccard_phase
            best_te_pr_each = test_precision_each_phase
            best_te_re_each = test_recall_each_phase
        ...
        logging.info(f"best_test_acc           :{best_te_acc:.4f}")
        ...
        logging.info(f"best_test_precision     :{best_te_pr:.4f}")
        logging.info(f"best_test_recall         :{best_te_re:.4f}")
        logging.info(f"best_test_jaccard        :{best_te_ji:.4f}")
        logging.info(f"best_test_precision_each:{best_te_pr_each}")
        logging.info(f"best_test_recall_each   :{best_te_re_each}")

def main():
    train_dataset, val_dataset, test_dataset = get_data(image_root_path, label_dir, train_vid_list_file, val_vid_list_file, test_vid_list_file, actions_dict)
