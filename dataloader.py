...
class VideoBoundaryDataset(data.Dataset):
    def __init__(self, vid_list_file, num_classes, actions_dict, gt_path, features_path, device, bd_ratio=0.05):
    ...
    def __getitem__(self, index):
        ...
        vid_name = self.list_of_examples[index]
        ...
        return feature_tensor, target_tensor, mask, match_score, vid_name

    def __len__(self):
        return len(self.list_of_examples)

    def _get_base_data(self, index):
        features = np.load(self.features_path + self.list_of_examples[index][:-4] + '.npy')
        file_ptr = open(self.gt_path + self.list_of_examples[index] + '.txt', 'r')
    ...
    def _get_train_label(self, index, target_tensor, anchor_xmin, anchor_xmax):
        ...
        gt_starts, gt_ends = gt_starts.astype(float), gt_ends.astype(float)
